...
export async function createScan(projectId: string, scanners: string[]): Promise<ScanRow> {
  const result = await client.query<ScanRow>(
    'INSERT INTO scans (project_id, scanners, status) VALUES ($1, $2, $3) RETURNING id, project_id, scanners, status, error_log, started_at, completed_at',
...
export async function updateScanStatus(
  ...
  const query = `
    UPDATE scans
    SET status = $2, started_at = $3, completed_at = $4, error_log = $5
    WHERE id = $1
  `;
  await client.query(query, [scanId, status, startedAt || null, completedAt || null, errorLog || null]);
}
